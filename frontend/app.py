...
#             submitted = st.form_submit_button("Create Account", use_container_width=True, key="signup_submit")
...
# def routine_page():
#     """Personalized skincare routine with detailed product info"""
#     st.title("✨ Your Personalized Skincare Routine")
#     st.caption(f"Hello {st.session_state.current_user.split('@')[0]}! Here's your customized routine")
...
#     # Refresh button
#     if st.button("🔄 Refresh with Latest Products", use_container_width=True):
#         st.session_state.routine_data = None
#         st.session_state.trending_products = None
#         st.rerun()
...
#         with st.spinner("🧪 Creating your personalized routine with detailed product info..."):
#             try:
#                 # Get AM routine
...
#     # Display routine in two columns
...
#         with am_col:
#             st.subheader("🌞 Morning (AM) Routine")
#             if am_data and "routine" in am_data:
#                 for step in am_data["routine"]:
#                     step_name = step["step"].replace("AM ", "")
#                     st.markdown(f"**{step_name.capitalize()}**")
                    
#                     # Check if products exist
#                     if "products" in step and step["products"]:
#                         for product in step["products"]:
#                             # Use product name instead of brand/title
#                             with st.expander(f"{product.get('name', 'Product')}", expanded=True):
#                                 # Display benefits and links
#                                 st.markdown(f"✨ **Benefits:** {product.get('benefits', '')}")
#                                 st.markdown(f"🔍 [Source]({product.get('source', '#')})")
#                                 st.markdown(f"🛒 [Purchase Link]({product.get('purchase_link', '#')})")
#                     else:
#                         st.warning("No product recommendation available")
        
#         with pm_col:
#             st.subheader("🌙 Evening (PM) Routine")
#             if pm_data and "routine" in pm_data:
#                 for step in pm_data["routine"]:
#                     step_name = step["step"].replace("PM ", "")
#                     st.markdown(f"**{step_name.capitalize()}**")
...
#     # Korean products section - updated field names
#     st.markdown("---")
#     st.subheader("🇰🇷 Trending Korean Skincare for You")
    
#     if st.button("Load Korean Skincare Trends", key="load_korean_trends"):
#         with st.spinner("🔍 Finding the best Korean products for your skin..."):
#             try:
#                 response = requests.post(
#                     f"{API_BASE_URL}/trending-korean-products",
#                     headers=get_auth_header()
#                 )
#                 if response.status_code == 200:
#                     st.session_state.korean_products = response.json()
#                 else:
#                     st.error("Failed to load Korean products")
#             except Exception as e:
#                 st.error(f"Error: {str(e)}")
    
#     if st.session_state.get("korean_products"):
#         for product in st.session_state.korean_products:
#             # Use title instead of name
#             with st.expander(f"{product.get('title', 'Korean Product')} 🇰🇷", expanded=True):
#                 st.markdown(f"**Type:** {product.get('type', '').capitalize()}")
#                 # Use snippet instead of description
#                 st.markdown(f"**Description:** {product.get('snippet', '')}")
#                 st.markdown(f"🔍 [Source]({product.get('source', '#')})")
#                 st.markdown(f"🛒 [Purchase Link]({product.get('purchase_link', '#')})")
#                 st.markdown("---")
#     else:
#         st.info("Click 'Load Korean Skincare Trends' to see recommendations")
